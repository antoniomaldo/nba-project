...
//        return transformToMap(teamModelOutput);
        ...
        String teamName = team.getPlayers().get(0).getTeam();
        System.out.println("Simulating for team " + teamName);
...
//        double teamFgExp = team.stream().filter(p->p.getCumFgAttemptedPerGame() != null). mapToDouble(p->p.getCumFgAttemptedPerGame()).sum();
        ...
        double total = prePopPlayerList.stream().mapToDouble(i->i.getBaseFgAttemptedPred()).sum();
        for(PrePopPlayer player : prePopPlayerList){
            double basePred = player.getBaseFgAttemptedPred();
            System.out.println(player.getName() + ", " + basePred + ", "+ total);
        }
