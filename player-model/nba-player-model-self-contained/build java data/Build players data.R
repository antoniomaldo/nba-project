...
library(arm)
library(stringr)
source("C:\\Users\\aostios\\Documents\\NBA\\nba-player-model-self-contained\\build java data\\Utils.R")
...
getPlayersData <- function(){
  ...
  allPlayers$cumSetOfFts <- unlist(aggregate(setOfFts ~ seasonYear + PlayerId, allPlayers, function(x) {cumsum(x)})$setOfFts)
  allPlayers$cumExtraFt <- unlist(aggregate(extraFt ~ seasonYear + PlayerId, allPlayers, function(x) {cumsum(x)})$extraFt)
  
  #Last season averages
  lastYear <- getPlayersForYear(2022)
  lastYear$setOfFts <- (lastYear$FT.Attempted - (lastYear$FT.Attempted %% 2)) / 2
  lastYear$extraFt <- (lastYear$FT.Attempted %% 2)
  ...
  allPlayers$lastYearExtraProb <- allPlayers$LastYearExtraFts / allPlayers$LastYearNumbGames
  allPlayers$cumExtraProb <- allPlayers$cumExtraFt / allPlayers$gamesPlayedSeason
